...
export default function CategoryList() {
  ...
  async function SetCategoriesInitial() {
    ...
    for (var i = 0; i < 3; i++) {
      await api
        .post("/api/Categories", initialCategories[i], { headers })
  ...
  return (
    <Container className={classes.cardGrid} maxWidth="md">
      {loading ? (
            ...
            <Grid item key={category.id} xs={12} sm={6} md={4}>
              <Card className={classes.card}>
                <AutoPlaySwipeableViews>
                  {images.length > 0 ? (
                    images
                      .map((image) => image.categoryId === category.id)
                      .filter((image) => image)
        ...
        <NoResults response={"Problemas no servidor, nehuma categoria encontrada..."} />
      )}
      {/* {loading?null:
