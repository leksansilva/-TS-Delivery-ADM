...
export default function OrderList(props) {
  ...
  const currentTab = {
    ...
    "/Pedidos/Pronto": "Pronto",
    "/Pedidos/SaiuParaEntrega": "que Saiu para Entrega",
    "/Pedidos/Entregue": "Entregue",
    "/Pedidos/NaoEntregue": "Não Entregue",
  ...
  async function handleOrderStatus(id) {
    ...
    if (newStatus <= 6) {
      api
        .put(`/api/Orders/${id}`, newOrderStatus, { headers })
        .then((response) => {
  ...
  async function handleAnotherOption(id) {
    ...
    if (anotherOption === 10) {
      api
        .delete(`/api/Orders?id=${id}`, { headers })
        .then((response) => {
          if (response.status === 200) {
            newEvent(true);
          }
              ...
              <Grid item container xs direction="column">
                <Grid item xs container>
                  <Grid item xs>
                    ...
                    <Chip
                      color={
                        order.paymentTypeId === 1 ? "primary" : "secondary"
                      }
                      size="small"
                      label={
                    ...
                    <Typography variant="subtitle2">
                      ...
                      {order.address.addon ? ", " + order.address.addon : ""}
                    </Typography>
                    <Typography variant="subtitle2" color="textSecondary">
                      Pedido realizado {ArrivalTime(order.shoppingTime)} h
                    </Typography>
                  </Grid>
                  <Grid item>
                ...
                <Grid item container direction="column">
                  ...
                  <Grid item container>
                    ...
                    <Grid item>
                      ...
                      <Button
                        size="large"
                        onClick={() => handleAnotherOption(order.id)}
                        color="secondary"
                      >
                        {props.button2}
                      </Button>
